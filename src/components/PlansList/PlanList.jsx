...
console.log("CAMBIOS DETECTADOS", cambiosDetectados)
  useEffect(() => {
    ...
    const eliminarItem = (prodIndex, planIndex, tipo, tratamientoIndex = null) => {
    ...
    const handleChange = (fieldKey, value, planIndex, tipo, itemIndex, tratamientoIndex = null) => {
        const copiaPlan = JSON.parse(JSON.stringify(plans[planIndex]));
        let item;
                            ...
                            {columnasPDF.map((col) => {
                              ...
                              const valor = maq[col.key] ?? "";
                                                    ...
                                                    <div key={col.key} className="flex-1 px-2">
                                                        ...
                                                        {enEdicion && fieldDef ? (
                                                            fieldDef.type === "select" ? (
                                                                <ListaDesplegable
                                                                    value={valor}
                                                                    onChange={(e) => handleChange(col.key, e.target.value, planIdx, 'maquinarias', mIdx)}
                                                                    array={typeof fieldDef.options === "function" ? fieldDef.options(maq) : fieldDef.options}
                                                                />
                                                            ) : (
                                                                <Input
                                                                    type={fieldDef.type || "text"}
                                            ...
                                            {plan.maquinarias.length > 1 && (
                                                <Button
                                                    className="bg-red-500 hover:bg-red-600 text-white px-2 py-2 rounded-lg shadow"
                                                    onClick={() => eliminarItem(mIdx, planIdx, 'maquinarias')}
                                                >
                                                    <Trash2 />
                                                </Button>
                                            )}
